...
export function InviteTeamMember({ isOpen, onClose, zIndex }: ModalProp<ValidatingModalProps>) {
  ...
  const deriveNamesFromEmail = (email: string, first?: string, last?: string) => {
    ...
    const lastName = last && last.trim() !== "" ? last : "";
  ...
  const handleRoleChange = async (memberId: number, newRole: TeamMemberRoleEnum) => {
    try {
                    ...
                    {user?.email !== member.email && (
                      <>
                        ...
                        <Tooltip
                          ...
                          border="none"
                          opacity={1}
                          render={() => <MemberRoleTooltip currentRole={member.role} onRoleChange={(role) => handleRoleChange(member.id, role)} />}
                        />
                      </>
                    )}
                  </div>
